...
TEST (network, construction_with_specified_port)
{
	...
	EXPECT_EQ (port, node->listener.endpoint ().port ());
}
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node0->stats.count (nano::stat::type::message, nano::stat::detail::node_id_handshake, nano::stat::dir::in) >= initial + 1);
	ASSERT_TIMELY (5s, node1->stats.count (nano::stat::type::message, nano::stat::detail::node_id_handshake, nano::stat::dir::in) >= initial_node1 + 1);
...
TEST (tcp_listener, tcp_node_id_handshake)
{
	...
	auto bootstrap_endpoint (system.nodes[0]->listener.endpoint ());
...
TEST (tcp_listener, DISABLED_tcp_listener_timeout_empty)
{
	...
	socket->async_connect (node0->listener.endpoint (), [&connected] (boost::system::error_code const & ec) {
		ASSERT_FALSE (ec);
		connected = true;
	});
	...
	{
		{
			nano::lock_guard<nano::mutex> guard (node0->listener.mutex);
			disconnected = node0->listener.connections.empty ();
		}
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<nano::client_socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (nano::transport::map_tcp_to_endpoint (node0->listener.endpoint ())));
	nano::node_id_handshake node_id_handshake{ nano::dev::network_params.network, cookie, boost::none };
	auto channel = std::make_shared<nano::transport::channel_tcp> (*node0, socket);
	socket->async_connect (node0->listener.endpoint (), [&node_id_handshake, channel] (boost::system::error_code const & ec) {
		ASSERT_FALSE (ec);
		channel->send (node_id_handshake, [] (boost::system::error_code const & ec, size_t size_a) {
			ASSERT_FALSE (ec);
		});
	});
	ASSERT_TIMELY (5s, node0->stats.count (nano::stat::type::network_server, nano::stat::detail::node_id_handshake) != 0);
	{
		nano::lock_guard<nano::mutex> guard (node0->listener.mutex);
		ASSERT_EQ (node0->listener.connections.size (), 1);
	}
	bool disconnected (false);
