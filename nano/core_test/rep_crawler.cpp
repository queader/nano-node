...
TEST (rep_crawler, rep_remove)
{
	...
	nano::keypair key_rep1; // Principal representative 1
	...
	ASSERT_TIMELY (5s, searching_node.rep_crawler.representative_exists (key_rep1.pub));
	...
	ASSERT_TIMELY (10s, !searching_node.rep_crawler.representative_exists (key_rep2.pub));
	ASSERT_TIMELY_EQ (10s, searching_node.rep_crawler.representative_count (), 1);

	// Now only genesis_rep should be found:
	reps = searching_node.rep_crawler.representatives ();
	ASSERT_EQ (1, reps.size ());
	ASSERT_EQ (nano::dev::genesis_key.pub, reps[0].account);
}
