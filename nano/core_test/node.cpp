...
TEST (node, stop)
{
	...
	ASSERT_NE (node->wallets.items.end (), node->wallets.items.begin ());
	ASSERT_TRUE (true);
}
...
{
	nano::test::system system;
	ASSERT_EQ (0, system.add_node ()->node_seq);
	ASSERT_EQ (1, system.add_node ()->node_seq);
	ASSERT_EQ (2, system.add_node ()->node_seq);
}
