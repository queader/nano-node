...
TEST (async, condition)
{
	...
	nano::async::condition condition{ strand };

	auto fut = asio::co_spawn (
	strand,
	[&] () -> asio::awaitable<void> {
		co_await condition.wait_for (std::chrono::seconds{ 10 });
	},
	asio::use_future);
...
TEST (async, condition_expire)
{
	...
	strand,
	[&] () -> asio::awaitable<void> {
		co_await condition.wait_for (std::chrono::seconds{ 1 });
	},
	asio::use_future);
