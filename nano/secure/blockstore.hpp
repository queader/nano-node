...
public:
	...
	virtual void votes_replay_for_each_par (std::function<void (nano::read_transaction const &, nano::store_iterator<nano::votes_replay_key, nano::vote>, nano::store_iterator<nano::votes_replay_key, nano::vote>)> const & action_a) const = 0;
	...
	virtual nano::store_iterator<nano::votes_replay_key, nano::vote> vote_replay_begin (nano::transaction const & transaction_a) const = 0;
	virtual nano::store_iterator<nano::votes_replay_key, nano::vote> vote_replay_end () const = 0;
...
std::unique_ptr<nano::block_store> make_vote_store (nano::logger_mt & logger, boost::filesystem::path const & path, bool open_read_only = false, bool add_db_postfix = false, nano::rocksdb_config const & rocksdb_config = nano::rocksdb_config{}, nano::txn_tracking_config const & txn_tracking_config_a = nano::txn_tracking_config{}, std::chrono::milliseconds block_processor_batch_max_time_a = std::chrono::milliseconds (5000), nano::lmdb_config const & lmdb_config_a = nano::lmdb_config{}, bool backup_before_upgrade = false);
}
