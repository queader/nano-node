...
public:
	...
	void put (nano::write_transaction const & transaction_a, nano::unchecked_key const & key_a, nano::unchecked_info const & info_a) override
	{
		...
		if (entries == nullptr && use_memory ())
		{
			...
			for_each (transaction_a, [&entries_new] (nano::unchecked_key const & key, nano::unchecked_info const & info) { entries_new->template get<tag_root> ().insert ({ key, info }); });
			clear (transaction_a);
