...
public:
	/**
	 * If there is an existing hash associated with the root, checks if it matches passed in `hash`
	 * If there is no existing hash associated with the root, inserts passed in `hash`
	 */
	virtual bool check_and_put (nano::write_transaction const & transaction_a, nano::qualified_root const & root_a, nano::block_hash const & hash_a) = 0;
