...
void nano::election::confirm_once (nano::unique_lock<nano::mutex> & lock_a, nano::election_status_type type_a)
{
	...
	if (state_m.exchange (nano::election::state_t::confirmed) != nano::election::state_t::confirmed && (node.active.election_winner_details.count (status_m.winner->hash ()) == 0))
	{
		node.active.election_winner_details.emplace (status_m.winner->hash (), shared_from_this ());
