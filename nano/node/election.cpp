...
void nano::election::confirm_if_quorum (nano::unique_lock<nano::mutex> & lock_a)
{
	...
	{
		if (node.canary.final_votes () && !is_quorum.exchange (true) && node.config.enable_voting && node.wallets.reps ().voting > 0)
		{
		...
		if (!node.canary.final_votes () || final_weight >= node.online_reps.delta ())
		{
