...
public:
	...
	virtual bool operator== (nano::channel const &) const = 0;
	...
	virtual nano::transport_type get_type () const = 0;
	virtual bool max ()
	{
		return false;
	}
...
{
template <>
struct hash<::nano::channel>
{
	std::size_t operator() (::nano::channel const & channel_a) const
	{
		return channel_a.hash_code ();
	}
};
template <>
struct equal_to<std::reference_wrapper<::nano::channel const>>
{
	bool operator() (std::reference_wrapper<::nano::channel const> const & lhs, std::reference_wrapper<::nano::channel const> const & rhs) const
	{
...
struct hash<::nano::channel>
{
	std::size_t operator() (::nano::channel const & channel_a) const
	{
		std::hash<::nano::channel> hash;
		return hash (channel_a);
	}
};
template <>
struct hash<std::reference_wrapper<::nano::channel const>>
{
	std::size_t operator() (std::reference_wrapper<::nano::channel const> const & channel_a) const
	{
		std::hash<::nano::channel> hash;
