...
/**
 * Server side of a frontier request. Created when a network_server receives a frontier_req message and exited when end-of-list is reached.
 */
class frontier_req_server final : public std::enable_shared_from_this<nano::frontier_req_server>
{
public:
	frontier_req_server (std::shared_ptr<nano::network_server> const &, std::unique_ptr<nano::frontier_req>);
	void send_next ();
