...
void nano::bootstrap_ascending::seed ()
{
	node.logger.always_log ("Seeding ascending bootstrap...");
	...
	node.logger.always_log ("Finished seeding ascending bootstrap");
}
...
{
	nano::asc_pull_req message{ node.network_params.network };
	message.id = tag.id;
	...
	bool sent = channel->send (message, [this, tag] (boost::system::error_code const & ec, std::size_t size) {
		if (ec)
		{
...
void nano::bootstrap_ascending::process (const nano::asc_pull_ack & message, async_tag const & tag)
{
	...
	{
		stats.add (nano::stat::type::bootstrap_ascending, nano::stat::detail::blocks, nano::stat::dir::in, message.blocks ().size ());
...
bool nano::bootstrap_ascending::verify (const nano::asc_pull_ack & message, const nano::bootstrap_ascending::async_tag & tag) const
{
	auto blocks = message.blocks ();

	debug_assert (!message.blocks ().empty ());
	auto first = message.blocks ().front ();

	nano::block_hash previous_hash{};

	// `start` should correspond to either previous block or account
	if (first->previous () == tag.start)
	{
		// Pass
		previous_hash = tag.start.as_block_hash ();
	}
	...
	return true; // Blocks pass verification
}
