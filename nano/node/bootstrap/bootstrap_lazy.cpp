...
//	std::size_t max_keys (node->flags.disable_legacy_bootstrap ? 4 * 1024 : 1024);
	...
	//		inserted = true;
	//	}
	if (lazy_keys.size () < max_keys)
	{
		if (lazy_keys.find (hash_or_account_a.as_block_hash ()) == lazy_keys.end ())
		{
			lazy_keys.insert (hash_or_account_a.as_block_hash ());
			lazy_pulls.emplace_back (hash_or_account_a, confirmed ? lazy_retry_limit_confirmed () : node->network_params.bootstrap.lazy_retry_limit);
			lock.unlock ();
...
void nano::bootstrap_attempt_lazy::lazy_requeue (nano::block_hash const & hash_a, nano::block_hash const & previous_a)
{
	...
	//	}
	node->bootstrap_initiator.connections->requeue_pull (nano::pull_info (hash_a, hash_a, previous_a, incremental_id, 0, node->network_params.bootstrap.lazy_destinations_retry_limit));
}
...
void nano::bootstrap_attempt_lazy::lazy_pull_flush (nano::unique_lock<nano::mutex> & lock_a)
{
	...
	if (pulling < max_pulls)
	{
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			//				node->bootstrap_initiator.connections->add_pull (nano::pull_info (pull_start.first, pull_start.first.as_block_hash (), nano::block_hash (0), incremental_id, 0, pull_start.second));
			//				++pulling;
			//				++count;
			//				lock_a.lock ();
			//			}
...
//{
//	debug_assert (false && "lazy_backlog_cleanup");
//
//	uint64_t read_count (0);
//	auto transaction (node->store.tx_begin_read ());
