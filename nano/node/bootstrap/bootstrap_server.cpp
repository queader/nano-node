...
void nano::bootstrap_server::received_message (std::unique_ptr<nano::message> message)
{
	...
	bool should_continue = process_message (std::move (message));
	if (should_continue)
	{
		receive_message ();
	}
...
bool nano::bootstrap_server::process_message (std::unique_ptr<nano::message> message)
{
	...
	else if (is_bootstrap_connection ())
	{
...
{
	server->to_bootstrap_connection ();
}
...
{
	if (!server.node->network_params.work.validate_entry (*message.block))
	{
		process = true;
	}
	else
	{
		server.node->stats.inc_detail_only (nano::stat::type::error, nano::stat::detail::insufficient_work);
	}
}
...
// TODO: Periodically call timeout
...
{
	return socket->type () == nano::socket::type_t::undefined || socket->type () == nano::socket::type_t::realtime_response_server;
}
