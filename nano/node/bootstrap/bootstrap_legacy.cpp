...
bool nano::bootstrap_attempt_legacy::request_frontier (nano::unique_lock<nano::mutex> & lock_a, bool first_attempt)
{
	...
	if (connection_l && !stopped)
	{
		...
		if (!result)
		{
			node->nlogger.debug (nano::log::tag::bootstrap_legacy, "Completed frontier request, {} out of sync accounts according to {}", account_count.load (), connection_l->channel->to_string ());
		}
		else
		{
...
void nano::bootstrap_attempt_legacy::run ()
{
	...
	while (still_pulling ())
	{
		...
		{
			node->nlogger.debug(nano::log::tag::bootstrap_legacy, "Requesting new frontiers after: {}", start_account.to_account ());
	...
	if (!stopped)
	{
		node->nlogger.debug(nano::log::tag::bootstrap_legacy, "Completed legacy pulls");
