...
/**
 * Server side of a bulk_pull request. Created when network_server receives a bulk_pull message and is exited after the contents
...
{
public:
	bulk_pull_server (std::shared_ptr<nano::network_server> const &, std::unique_ptr<nano::bulk_pull>);
	...
	std::shared_ptr<nano::network_server> connection;
...
{
public:
	bulk_pull_account_server (std::shared_ptr<nano::network_server> const &, std::unique_ptr<nano::bulk_pull_account>);
	void set_params ();
