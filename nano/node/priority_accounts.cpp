...
void nano::priority_accounts::activate (nano::store::transaction const & transaction, nano::account const & account, nano::account_info const & account_info)
{
	...
	auto const head_balance = node.ledger.balance_safe (transaction, account_info.head, error);

	auto const head_block = node.ledger.head_block (transaction, account);
	auto const previous_balance = node.ledger.balance_safe (transaction, head_block->previous (), error);

	auto const conf_balance = node.ledger.balance_safe (transaction, conf_info.frontier, error);
