...
nano::vote_storage::vote_storage (nano::node & node_a, nano::store & vote_store_a, nano::network & network_a, nano::ledger & ledger_a, nano::stats & stats_a) :
	node{ node_a },
	...
	broadcast_queue{ stats, nano::stat::type::vote_storage_broadcast, nano::thread_role::name::vote_storage, /* threads */ 2, 1024 * 64, 1024 }
{
...
void nano::vote_storage::process_batch (decltype (store_queue)::batch_t & batch)
{
	...
	for (auto & vote : batch)
	{
		...
		if (result > 0)
		{
			...
			stats.add (nano::stat::type::vote_storage_store, nano::stat::detail::stored_votes, nano::stat::dir::in, result);
		}
	}
}
...
void nano::vote_storage::reply (const nano::vote_storage::vote_list_t & votes, const std::shared_ptr<nano::transport::channel> & channel)
{
	...
	for (auto & vote : votes)
	{
		...
		nano::transport::buffer_drop_policy::no_limiter_drop, nano::bandwidth_limit_type::vote_storage);
	}
}
	...
	for (auto & vote : votes)
	{
		...
		if (enable_random_broadcast)
		{
			for (auto const & channel : random_nodes)
			{
				...
				nano::transport::buffer_drop_policy::limiter, nano::bandwidth_limit_type::vote_storage);
			}
		}
	}
}
...
nano::vote_storage::vote_list_t nano::vote_storage::query_hash (const nano::transaction & vote_transaction, const nano::block_hash & hash, std::size_t count_threshold)
{
...
std::pair<nano::vote_storage::vote_list_t, nano::block_hash> nano::vote_storage::query_frontier (nano::transaction const & ledger_transaction, nano::transaction const & vote_transaction, const nano::block_hash & hash)
{
