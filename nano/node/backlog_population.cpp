...
void nano::backlog_population::activate (nano::transaction const & transaction, nano::account const & account)
{
	...
	auto const account_info = store.account.get (transaction, account);
	if (!account_info)
	{
		return;
	}
	auto const conf_info = store.confirmation_height.get (transaction, account);

	// If conf info is empty then it means then it means nothing is confirmed yet
	if (!conf_info || conf_info->height < account_info->block_count)
	{
