...
public:
	...
	void prepare (std::vector<nano::representative> const & reps);
	...
	bool broadcast (std::shared_ptr<nano::block> const & candidate, std::unordered_map<nano::account, nano::vote_info> const & votes);
	/** Add an election that needs to be confirmed. Returns false if successfully added */
	size_t request (std::shared_ptr<nano::block> const & candidate, std::unordered_map<nano::account, nano::vote_info> const & votes);
...
private:
	...
	using root_hashes = std::vector<std::pair<nano::block_hash, nano::root>>;
	std::unordered_map<std::shared_ptr<nano::transport::channel>, root_hashes> requests;
};
}
