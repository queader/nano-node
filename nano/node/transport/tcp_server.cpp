...
void nano::transport::tcp_server::receive_message ()
{
	...
	message_deserializer->read (socket, [this_l = shared_from_this ()] (boost::system::error_code ec, std::unique_ptr<nano::message> message) {
		if (ec)
		{
			std::cerr << "tcp_server::receive_message: " << ec << "   " << ec.message ()
					  << " | "
					  << "status: " << nano::transport::message_deserializer::to_string (this_l->message_deserializer->status)
					  << " | "
...
void nano::transport::tcp_server::received_message (std::unique_ptr<nano::message> message)
{
	...
	{
		//		std::cerr << "tcp_server::receive_message status: " << nano::transport::message_deserializer::to_string (message_deserializer->status)
		//				  << " | "
