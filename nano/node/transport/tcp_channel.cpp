...
{
	sending_task = nano::async::task (strand, [this, /* guard lifetime */ self = shared_from_this ()] () -> asio::awaitable<void> {
		try
		{
...
asio::awaitable<void> nano::transport::tcp_channel::send_one (traffic_type type, tcp_channel_queue::entry_t const & item)
{
	...
	socket->async_write (
	buffer,
	[this_s = shared_from_this (), node = node.weak_from_this (), callback] (boost::system::error_code const & ec, std::size_t size) {
		if (auto node_l = node.lock ())
		{
			if (!ec)
			{
