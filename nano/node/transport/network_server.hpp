...
public:
	...
	nano::mutex mutex;
	std::unordered_map<nano::network_server *, std::weak_ptr<nano::network_server>> connections;
...
class network_server final : public std::enable_shared_from_this<nano::network_server>
{
public:
	network_server (std::shared_ptr<nano::socket>, std::shared_ptr<nano::node>, bool allow_bootstrap = true);
	~network_server ();
	void start ();
	void stop ();
...
private:
	...
	public:
		...
		explicit handshake_message_visitor (std::shared_ptr<network_server>);
	...
	private:
		std::shared_ptr<network_server> server;
	};
	...
	public:
		...
		explicit realtime_message_visitor (network_server &);
	...
	public:
		...
		explicit bootstrap_message_visitor (std::shared_ptr<network_server>);
