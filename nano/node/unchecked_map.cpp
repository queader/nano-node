...
void nano::unchecked_map::check_in_memory ()
{
	...
	if (entries == nullptr && use_memory ())
	{
		...
		for_each (
		[&entries_new] (nano::unchecked_key const & key, nano::unchecked_info const & info) { entries_new->template get<tag_root> ().insert ({ key, info }); }, [&] () { return entries_new->size () < mem_block_count_max; });
		clear ();
