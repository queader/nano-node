...
public:
	...
	unsigned network_threads{ std::max<unsigned> (4, nano::hardware_concurrency ()) };
	unsigned work_threads{ std::max<unsigned> (4, nano::hardware_concurrency ()) };
	...
	unsigned signature_checker_threads{ nano::hardware_concurrency () / 2 };
	bool enable_voting{ false };
	unsigned bootstrap_connections{ 4 };
	unsigned bootstrap_connections_max{ 64 };
	unsigned bootstrap_initiator_threads{ 1 };
	unsigned bootstrap_serving_threads{ std::max<unsigned> (2, nano::hardware_concurrency () / 2) };
