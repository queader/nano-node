...
public:
	vote_storage (nano::node &, nano::store & vote_store, nano::network &, nano::ledger &, nano::stats &);
	~vote_storage ();
...
private:
	...
	vote_list_t query_hash (nano::transaction const & vote_transaction, nano::block_hash const &, std::size_t count_threshold = 0);
	/** @return <votes, votes frontier> */
	std::pair<vote_list_t, nano::block_hash> query_frontier (nano::transaction const & ledger_transaction, nano::transaction const & vote_transaction, nano::block_hash const &);
...
private: // Flags
	...
	static bool constexpr enable_pr_broadcast = false;
