...
std::unique_ptr<nano::container_info_component> nano::collect_container_info (node & node, std::string const & name)
{
	...
	composite->add_component (collect_container_info (node.final_generator, "vote_generator_final")); // Decided on this name to have it next to normal generator when sorting stats alphabetically
	return composite;
}
